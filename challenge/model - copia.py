...
class DelayModel:
    ...
    def preprocess(self, data: pd.DataFrame, target_column: str = None) -> Union[Tuple[pd.DataFrame, pd.DataFrame], pd.DataFrame]:
        ...
        if 'Fecha-O' in self.data.columns and 'Fecha-I' in self.data.columns:
            self.data['period_day'] = self.data['Fecha-I'].apply(
                get_period_day)
            self.data['high_season'] = self.data['Fecha-I'].apply(
                is_high_season)
            self.data['min_diff'] = data.apply(get_min_diff, axis=1)
            threshold_in_minutes = 15
            self.data['delay'] = np.where(
                self.data['min_diff'] > threshold_in_minutes, 1, 0)
        ...
        columns_to_use = features.columns.intersection(expected_columns)
        ...
        if target_column and target_column in self.data.columns:
            ...
            target = pd.DataFrame([0] * len(data))
    ...
    def fit(self, features: pd.DataFrame, target: pd.DataFrame) -> None:
        ...
        xgb_model = xgb.XGBClassifier(random_state=1, learning_rate=0.01,
                                      scale_pos_weight=scale, max_depth=3, min_child_weight=1, gamma=0)
    ...
    def predict(self, features: pd.DataFrame) -> List[int]:
        # Utiliza get_model en lugar de self._model directamente.
    ...
    def get_model(self) -> Union[xgb.XGBClassifier, None]:
        if self._model is None:
            # Check if 'delay' is in self.data
            if 'delay' in self.data.columns:
                features, target = self.preprocess(self.data)
            else:
                # Load backup training dataset from CSV
                backup_data = pd.read_csv('G:/latam-challenge/data/data.csv')
                features, target = self.preprocess(backup_data)
    ...
    def data_analysis(self, data: pd.DataFrame) -> None:
        ...
        if 'DIANOM' in self.data.columns:
        ...
        if 'TIPOVUELO' in self.data.columns:
        ...
        if 'SIGLADES' in self.data.columns:
    ...
    def calculate_delay_rate_by_column(self, data: pd.DataFrame) -> None:
        if 'SIGLADES' in data.columns and 'delay' in data.columns:
        ...
        if 'OPERA' in data.columns and 'delay' in data.columns:
        ...
        if 'MES' in data.columns and 'delay' in data.columns:
        ...
        if 'DIANOM' in data.columns and 'delay' in data.columns:
        ...
        if 'high_season' in data.columns and 'delay' in data.columns:
